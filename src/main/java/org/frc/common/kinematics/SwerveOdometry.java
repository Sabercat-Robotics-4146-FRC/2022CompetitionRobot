package org.frc.common.kinematics;
 ...
 *
 * <p>Odometry allows a robot to track what it's position on the field is using the encoders on it's
 * swerve modules.
 */
public class SwerveOdometry {
   ...
   *
   * <p>This should be called when the gyroscope's angle is reset.
   *
   ...
   * @param moduleVelocities The velocities of the swerve modules. The modules must be in the same
   *     order that {@link SwerveKinematics} was given when it was instantiated.
  ...
  public RigidTransform2 update(Rotation2 gyroAngle, double dt, Vector2... moduleVelocities) {
    ...
    Vector2 newPosition = pose.translation.add(fieldOrientedVelocity.scale(dt));
