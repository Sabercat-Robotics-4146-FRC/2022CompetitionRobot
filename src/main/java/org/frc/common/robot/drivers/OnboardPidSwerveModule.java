...
import org.frc.common.control.PidConstants;
import org.frc.common.control.PidController;
import org.frc.common.drivers.SwerveModule;
...
public class OnboardPidSwerveModule extends SwerveModule {
   ...
   */
  public OnboardPidSwerveModule(
      Vector2 positionOffset,
      double angleOffset,
      MotorController angleMotor,
      MotorController driveMotor,
      DoubleSupplier angleEncoder,
      DoubleSupplier driveEncoder,
  ...
  /**
   * Sets the hardware update thread rate. The default rate is {@value
   * #DEFAULT_HARDWARE_UPDATE_RATE}.
   *
  ...
  /**
   * Gets the current rate that the hardware update thread is running at. This is not the rate
   * specified by {@link #setHardwareUpdateRate(double)} but instead is determined by how fast the
   * hardware thread is currently running.
   *
   ...
   *
   * <p>This should only be called by the constructor.
   *
