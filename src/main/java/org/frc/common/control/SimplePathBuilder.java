...
public final class SimplePathBuilder {
  ...
  public static final class ArcSegment extends PathSegment {
    ...
    public State calculate(double distance) {
      ...
      double angle =
          Vector2.getAngleBetween(deltaStart, deltaEnd).toRadians()
              * (clockwise ? -1.0 : 1.0)
              * percentage;
      return new State(
          ...
          deltaStart
              .rotateBy(Rotation2.fromRadians(angle + (clockwise ? -1.0 : 1.0) * 0.5 * Math.PI))
              .getAngle(),
  ...
  public static final class LineSegment extends PathSegment {
    ...
    @Override
    public State calculate(double distance) {
      return new State(start.add(delta.scale(distance / getLength())), delta.getAngle(), 0.0);
    }
