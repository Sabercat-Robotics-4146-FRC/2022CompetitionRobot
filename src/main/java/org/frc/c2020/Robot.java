...
public class Robot extends TimedRobot {
  ...
  private static final byte[] COMPETITION_BOT_MAC_ADDRESS =
      new byte[] {0x00, (byte) 0x80, 0x2f, 0x28, (byte) 0xc3, 0x33};
  private static final byte[] PRACTICE_BOT_MAC_ADDRESS =
      new byte[] {0x00, (byte) 0x80, 0x2f, 0x22, (byte) 0xd7, (byte) 0xba};
  ...
  static {
    ...
    if (!competitionBot && !practiceBot) {
      String[] macAddressStrings =
          macAddresses.stream().map(Robot::macToString).toArray(String[]::new);

      SmartDashboard.putStringArray("MAC Addresses", macAddressStrings);
      SmartDashboard.putString(
          "Competition Bot MAC Address", macToString(COMPETITION_BOT_MAC_ADDRESS));
      SmartDashboard.putString("Practice Bot MAC Address", macToString(PRACTICE_BOT_MAC_ADDRESS));

      // If something goes terribly wrong we still want to use the competition bot stuff in
      // competition.
      competitionBot = true;
    }
  ...
  public void robotPeriodic() {
    ...
    double distFromARedAngle =
        robotContainer
            .getDrivetrainSubsystem()
            .getPose()
            .rotation
            .rotateBy(Rotation2.fromDegrees(180).inverse())
            .toRadians();
    double distFromABlueAngle =
        robotContainer
            .getDrivetrainSubsystem()
            .getPose()
            .rotation
            .rotateBy(Rotation2.fromDegrees(90).inverse())
            .toRadians();
    double distFromBRedAngle =
        robotContainer
            .getDrivetrainSubsystem()
            .getPose()
            .rotation
            .rotateBy(Rotation2.fromDegrees(45).inverse())
            .toRadians();
    double distFromBBlueAngle =
        robotContainer
            .getDrivetrainSubsystem()
            .getPose()
            .rotation
