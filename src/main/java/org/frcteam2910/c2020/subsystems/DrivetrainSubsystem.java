...
public class DrivetrainSubsystem implements Subsystem, UpdateManager.Updatable {
    ...
    public static final DrivetrainFeedforwardConstants FEEDFORWARD_CONSTANTS = new DrivetrainFeedforwardConstants( // tune with sysid, view w/ .3190 meters per rotation
    0.70067,
    2.2741,
    0.16779
    );
    ...
    public void drive(Vector2 translationalVelocity, double rotationalVelocity, boolean isFieldOriented) {
        synchronized (stateLock) {
            // Vector2 slowTranslationalVelocity = new Vector2(translationalVelocity.x / 2, translationalVelocity.y / 2);
            driveSignal = new HolonomicDriveSignal(translationalVelocity.scale(0.5), rotationalVelocity, isFieldOriented);
        }
    }
