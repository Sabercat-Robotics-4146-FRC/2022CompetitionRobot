...
public class AimRobotCommand extends CommandBase {
    ...
    private final Vector2 translation = Vector2.ZERO;
       ...
       */
        ...
        if(limelight.getSeesTarget()) {
            ...
            if (Math.abs(horizontal_angle) <= 0.03) {
                ...
                rotationSpeed = (max - min) * (Math.pow((horizontal_angle / 27), 2)) + min; //Max angle, I think it is 27
                rotationSpeed = Math.copySign(rotationSpeed, horizontal_angle) ;
                //I think 1 is the upper bound for rotational speed, hence, if not, scaling, and ceiling is unnecessary
            }
        }
        else {
        ...
        if(rotationSpeed < -2*max || rotationSpeed > 2*max) end(true); //This should never happen, but, is a safety measure
        drivetrainSubsystem.drive(Vector2.ZERO, rotationSpeed, false);
        pastRotationSpeed = rotationSpeed;
    }
    @Override
    public void end(boolean interrupted) {
        drivetrainSubsystem.drive(translation, 0, false);
    }
    @Override
