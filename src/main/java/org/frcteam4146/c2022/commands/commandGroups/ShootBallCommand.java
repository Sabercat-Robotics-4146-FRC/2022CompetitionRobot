  ...
  public ShootBallCommand(Limelight limelight, Flywheel flywheel, Indexer indexer) {
    addCommands(
        new ParallelCommandGroup(
            new ToggleLimelightTrackingCommand(
                limelight, true), // toggles limelight tracking indefinitely
            new ToggleFlywheelCommand(
                flywheel,
                true,
                limelight
                    .getDistanceFromTarget())), // toggles flywheel on for entire duration of the
        // command
        new WaitCommand(2),
        new ToggleLimelightTrackingCommand(
            limelight, false), // disables limelight while ball is being loaded only
